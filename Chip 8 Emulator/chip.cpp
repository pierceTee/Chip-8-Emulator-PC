...
bool chip::load(const char *file_path)
{
	...
	if (rom == NULL)
	{
		std::cerr << "Could not open specified ROM" << std::endl;
		return false;
	}
	...
	if (rom_buffer == NULL)
	{
		std::cerr << "Could not allocate memory for specified ROM file" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "Specified ROM is too large to fit in memory" << std::endl;
		return false;
	}
...
void chip::emulateCycle()
{
	...
	case 0x0000:
		...
		default:
			std::cout << "Error: Undefined OpCode -> " << opcode << std::endl;
			exit(EXIT_FAILURE);
		}
		break;
